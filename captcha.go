...
//返回图片字节集
func (captcha *CaptchaImage) ImageBytes(imageFormat int) []byte {
	emptyBuff := bytes.NewBuffer(nil)
	if imageFormat == ImageFormatPng {
		png.Encode(emptyBuff, captcha.nrgba)
	}
	if imageFormat == ImageFormatJpeg {
		jpeg.Encode(emptyBuff, captcha.nrgba, &jpeg.Options{Quality: 100})
	}
	if imageFormat == ImageFormatGif {
		gif.Encode(emptyBuff, captcha.nrgba, &gif.Options{NumColors: 256})
	}
